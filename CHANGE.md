...
1. **📱 Android App 容器与原生扫码桥接 (`mobile/android/`)**
   ...
   - ** Web ➔ Native 原生删除同步**：在 [`common/js/app.js`](file:///Users/edy/Desktop/1hhq/1AItools/HqQrUtils/common/js/app.js) 的 `deleteRecord` 和 `clearAllHistory` 方法中，新增对 Android 原生 JS 桥接通道的调用。当在 Web 页中删除或清空历史记录时，自动触发 `AndroidNative.deleteWebRecordFromNative(content)` 与 `AndroidNative.clearAllNativeRecords()`，同步在 Android SQLite 数据库 (`ScanDatabase`) 中同步删除/清空记录，实现 Web 端与原生 `ScanHistoryActivity` 历史数据无缝同步删除。
   - **URL Scheme 唤起跳转配置 (`ScanActivity.kt`)**：将 `qrutils://app/scan` 的 URL Scheme `<intent-filter>` 唤起入口从 `MainActivity` 迁移配置到原生扫码组件 [`ScanActivity`](file:///Users/edy/Desktop/1hhq/1AItools/HqQrUtils/mobile/android/app/src/main/AndroidManifest.xml#L51-L68)，设置 `android:exported="true"`。外部触发 `qrutils://app/scan` 协议链接时将直接唤起原生极速扫码界面。
