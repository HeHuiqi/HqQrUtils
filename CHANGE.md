...
2. **二维码图片解码与识别 (QR Code Scanner & Decoder)**
   - 增加 **“生成二维码 / 识别二维码”** 模式切换 Tab。
   - 支持 **文件拖拽上传**、**点击选择图片** 以及 **快捷键 `Ctrl+V` / `Cmd+V` 粘贴截图识别**。
   - 融合 Chrome 原生 `BarcodeDetector` 与离线算法双引擎，秒级解析出图片中的文本或链接。
   - 识别结果支持**一键复制**、**直接在新标签页打开 URL**、或**一键导入生成器**重新编辑美化。

3. **中心 Logo 图标嵌入 (Center Logo Overlay)**
