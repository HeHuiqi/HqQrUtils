/**
 * Chrome Extension 应用控制器 (Extension App Controller)
 */
...
(function (window) {
    ...
    function init() {
        ...
        StorageAdapter.loadHistory((records) => {
    ...
    function stopCameraScan() {
        ...
        if (DOM.cameraViewport) DOM.cameraViewport.style.display = 'none';
        if (DOM.cameraBtnText) DOM.cameraBtnText.textContent = '开启摄像头扫描';
    }

    function initTheme() {
        const savedTheme = StorageAdapter.getTheme();
    ...
    function toggleTheme() {
        ...
        if (isDark) {
            document.body.classList.replace('theme-dark', 'theme-light');
            StorageAdapter.setTheme('light');
            ToastManager.show('已切换至浅色主题', 'info');
        } else {
            document.body.classList.replace('theme-light', 'theme-dark');
            StorageAdapter.setTheme('dark');
    ...
    function saveRecord(options) {
        ...
        StorageAdapter.saveHistory(historyRecords);
        return newRecord;
    }
    ...
    function selectHistoryRecord(record) {
        ...
        if (DOM.categorySelect) DOM.categorySelect.value = record.category || 'none';
    ...
    function copyImageToClipboard() {
        ...
        canvas.toBlob(blob => {
            ...
            if (navigator.clipboard && window.ClipboardItem) {
                ...
                }).catch(() => ToastManager.show('复制图片失败', 'error'));
            } else {
    ...
    function bindEvents() {
        ...
        DOM.decodeFileInput.addEventListener('change', (e) => {
            if (e.target.files.length > 0) processDecodeFile(e.target.files[0]);
        });
        ...
        DOM.dropZone.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer.files.length > 0) processDecodeFile(e.dataTransfer.files[0]);
        });
        ...
        DOM.copyDecodeTextBtn.addEventListener('click', () => copyTextToClipboard(DOM.decodeResultText.value));
        ...
        DOM.exportHistoryBtn.addEventListener('click', () => {
            if (StorageAdapter.exportJSON(historyRecords)) {
        ...
        DOM.importFileInput.addEventListener('change', (e) => {
            if (e.target.files.length > 0) {
                StorageAdapter.importJSON(e.target.files[0], (err, imported) => {
                    if (err) {
